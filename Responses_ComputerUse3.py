...
def take_action(action, action_type, call_id, pending_checks, computer: LocalPlaywrightComputer):
    ...
    call_output = create_call_output(call_id, screenshot_base64, pending_checks, current_url)
...
def main():
    ...
    with LocalPlaywrightComputer() as computer:
        ...
        while True:  # get user input forever
            user_input = input("\n Working ...  What would you like to browse today?  ")
            ...
            while True:  # keep looping until we get a final response
                ...
                for item in response.output:
                    print('\n counting',ctr,item.type)
                    ctr=ctr+1
                    if item.type == 'computer_call':
                        
                        items += take_action(item.action, item.action.type, item.call_id, item.pending_safety_checks, computer)
                # Write the items list to a file
        try:
            with open('computer_use_items.txt', 'w') as file:
                # Convert the items list to a formatted string representation
                for i, item in enumerate(items):
                    ...
                    file.write("\n\n")
            print(f"\nItems list written to computer_use_items.txt")
        except Exception as e:
